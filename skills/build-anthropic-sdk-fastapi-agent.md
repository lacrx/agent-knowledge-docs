...
- [ ] `pip install fastapi uvicorn anthropic pydantic` completed
- [ ] Each tool has a unique `name` and clear `description`
- [ ] Tool handlers return strings (JSON-serialized for structured data)
- [ ] `input_schema` follows JSON Schema spec with `required` array
- [ ] Tool loop checks `stop_reason == "end_turn"` to exit
- [ ] Tool loop has a `max_turns` guard against infinite loops
- [ ] CORS middleware configured for frontend origin
- [ ] `Field(description=...)` on all Pydantic model fields
